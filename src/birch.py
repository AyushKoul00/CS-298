...
OUTPUT_DIR = SAVED_MODELS_DIR / "birch"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
FILE_NAME = "mean_embedding_per_file.pkl"
# FILE_NAME = "opcode_distribution_embeddings.pkl"
NORMALIZE_EMBEDDINGS = False

# Default Birch hyperparameters (to be tuned by Optuna)
BIRCH_BRANCHING_FACTOR = 50  # fixed
# The study will search for:
#    threshold: float in [0.1, 1.0]
#    n_clusters: int in [2, 20]

# --- Logging Setup ---
logging.basicConfig(
    filename=OUTPUT_DIR / "birch.log",
    level=logging.INFO,
...
def perform_birch(embeddings: np.ndarray, threshold: float, n_clusters: int) -> np.ndarray:
    logger.info("Running Birch clustering with threshold=%.2f, branching_factor=%d, n_clusters=%d",
                threshold, BIRCH_BRANCHING_FACTOR, n_clusters)
...
def plot_combined_2d(embeddings_2d: np.ndarray,
    ...
    ax_true.set_title(f"{method}: True Labels")
    ...
    colors = [cluster_color_map[label] if label != -1 else (0.5, 0.5, 0.5, 0.7) for label in cluster_labels]
    ax_pred.scatter(embeddings_2d[:, 0], embeddings_2d[:, 1],
                    c=colors, alpha=0.7, s=15)
    ax_pred.set_title(f"{method}: Predicted Clusters")
...
def plot_combined_3d(embeddings_3d: np.ndarray,
                     true_labels: np.ndarray,
                     cluster_labels: np.ndarray,
                     method: str,
                     save_path: Path) -> None:
...
def objective(trial: optuna.Trial) -> float:
    ...
    cluster_labels = perform_birch(processed, threshold=threshold, n_clusters=n_clusters)
...
# --- Main Execution ---
def main() -> None:
    study = optuna.create_study(direction="maximize", study_name=f"birch_{MODEL}",
                                storage=f"sqlite:///{OUTPUT_DIR / 'birch.db'}")
    ...
    cluster_labels = perform_birch(processed, threshold=best_params["threshold"], n_clusters=best_params["n_clusters"])
    ...
    plot_combined_2d(emb_pca, true_labels, cluster_labels, method="PCA", save_path=OUTPUT_DIR)
    
    tsne_2d = TSNE(n_components=2, n_jobs=-1)
    emb_tsne = tsne_2d.fit_transform(processed)
    plot_combined_2d(emb_tsne, true_labels, cluster_labels, method="t-SNE", save_path=OUTPUT_DIR)
    
    umap_2d = umap.UMAP(n_components=2, n_jobs=-1)
    emb_umap = umap_2d.fit_transform(processed)
    plot_combined_2d(emb_umap, true_labels, cluster_labels, method="UMAP", save_path=OUTPUT_DIR)
    
    # Combined 3D visualizations
    pca_3d = PCA(n_components=3, random_state=42)
    emb_pca_3d = pca_3d.fit_transform(processed)
    plot_combined_3d(emb_pca_3d, true_labels, cluster_labels, method="PCA", save_path=OUTPUT_DIR)
    
    tsne_3d = TSNE(n_components=3, n_jobs=-1)
    emb_tsne_3d = tsne_3d.fit_transform(processed)
    plot_combined_3d(emb_tsne_3d, true_labels, cluster_labels, method="t-SNE", save_path=OUTPUT_DIR)
    
    umap_3d = umap.UMAP(n_components=3, n_jobs=-1)
    emb_umap_3d = umap_3d.fit_transform(processed)
    plot_combined_3d(emb_umap_3d, true_labels, cluster_labels, method="UMAP", save_path=OUTPUT_DIR)
