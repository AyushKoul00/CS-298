...
# n_clusters: Desired number of clusters; linkage: method to compute distances
HC_N_CLUSTERS: int = 10
HC_LINKAGE: str = "ward"  # Options include "ward", "complete", "average", "single"

# --- Logging Setup ---
logging.basicConfig(
    filename=SAVED_MODELS_DIR / 'hierarchical.log',  # Log file path
...
def perform_hierarchical(embeddings: np.ndarray,
                           n_clusters: int = HC_N_CLUSTERS,
                           linkage: str = HC_LINKAGE) -> np.ndarray:
    """
    Perform hierarchical (agglomerative) clustering on the embeddings.
...
def plot_true_labels(projected_embeddings: np.ndarray, true_labels: np.ndarray, save_path: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(save_path / "true_labels_visualization_hierarchical.png", dpi=300)
    plt.show()


def plot_cluster_assignments(projected_embeddings: np.ndarray, cluster_labels: np.ndarray, save_path: Path) -> None:
    """Visualize the embeddings colored by hierarchical cluster assignments."""
    plt.figure(figsize=(8, 8))
    ...
    plt.title(f"Hierarchical Clustering (n_clusters={HC_N_CLUSTERS}, linkage={HC_LINKAGE})")
    plt.xlabel("Component 1")
    plt.ylabel("Component 2")
    legend_handles = [
    ...
    plt.tight_layout()
    plt.savefig(save_path / "cluster_visualization_hierarchical.png", dpi=300)
    plt.show()
...
def plot_pca_hierarchical(embeddings: np.ndarray, n_clusters: int = HC_N_CLUSTERS, linkage: str = HC_LINKAGE) -> None:
    """
    Visualize hierarchical clustering on PCA-reduced embeddings.
    ...
    try:
        ...
        logger.info("PCA-Hierarchical Silhouette Coefficient: %.3f", silhouette)
    except Exception as e:
        logger.warning("Silhouette score calculation failed: %s", e)
    ...
    plt.title(f"PCA Hierarchical Clustering (n_clusters={n_clusters}, linkage={linkage})")
...
def main() -> None:
    ...
    cluster_labels = perform_hierarchical(processed_embeddings, n_clusters=HC_N_CLUSTERS, linkage=HC_LINKAGE)
    ...
    # Visualize true labels and hierarchical cluster assignments.
    ...
    # PCA-based hierarchical clustering visualization.
    plot_pca_hierarchical(processed_embeddings, n_clusters=HC_N_CLUSTERS, linkage=HC_LINKAGE)
