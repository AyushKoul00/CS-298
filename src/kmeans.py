...
# All outputs are stored in the "kmeans" folder.
OUTPUT_DIR = SAVED_MODELS_DIR / "kmeans"
...
# --- Logging Setup ---
logging.basicConfig(
    filename=OUTPUT_DIR / "kmeans.log",
    level=logging.INFO,
...
def plot_combined_2d(embeddings_2d: np.ndarray, true_labels: np.ndarray,
                     cluster_labels: np.ndarray, method: str,
                     save_path: Path, model_name: str) -> None:
...
# --- Main Execution ---
def main() -> None:
    study = optuna.create_study(direction="maximize", study_name=f"kmeans_{MODEL}",
                                storage=f"sqlite:///{OUTPUT_DIR / 'kmeans.db'}")
    ...
    cluster_labels = perform_kmeans(processed, n_clusters=best_params["n_clusters"])
