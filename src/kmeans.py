...
# KMeans Hyperparameters (Global Constants)
KMEANS_N_CLUSTERS: int = 10  # Set desired number of clusters

# --- Logging Setup ---
logging.basicConfig(
    filename=SAVED_MODELS_DIR / 'kmeans.log',  # Log file path
...
def perform_kmeans(embeddings: np.ndarray, n_clusters: int = KMEANS_N_CLUSTERS) -> np.ndarray:
...
def plot_true_labels(projected_embeddings: np.ndarray, true_labels: np.ndarray, save_path: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(save_path / "true_labels_visualization_kmeans.png", dpi=300)
    plt.show()


def plot_cluster_assignments(projected_embeddings: np.ndarray, cluster_labels: np.ndarray, save_path: Path) -> None:
    """Visualize the embeddings colored by KMeans cluster assignments."""
    plt.figure(figsize=(8, 8))
    ...
    plt.title(f"KMeans Clustering (n_clusters={KMEANS_N_CLUSTERS})")
    plt.xlabel("Component 1")
    plt.ylabel("Component 2")
    legend_handles = [
    ...
    plt.tight_layout()
    plt.savefig(save_path / "cluster_visualization_kmeans.png", dpi=300)
    plt.show()
...
def plot_pca_kmeans(embeddings: np.ndarray, n_clusters: int = KMEANS_N_CLUSTERS) -> None:
    """
    Visualize KMeans clustering on PCA-reduced embeddings.
    ...
    try:
        ...
        logger.info("PCA-KMeans Silhouette Coefficient: %.3f", silhouette)
    except Exception as e:
        logger.warning("Silhouette score calculation failed: %s", e)
    ...
    plt.title(f"PCA-KMeans Clustering (n_clusters={n_clusters})")
...
def main() -> None:
    ...
    # Perform clustering.
    cluster_labels = perform_kmeans(processed_embeddings, n_clusters=KMEANS_N_CLUSTERS)
    ...
    # PCA-based KMeans clustering visualization.
    plot_pca_kmeans(processed_embeddings, n_clusters=KMEANS_N_CLUSTERS)
